...
def get_memories(current_user):
    ...
    memories = supabase.get_memories(category)
...
def manage_memory(current_user):
    ...
    if 'id' in memory_data:
        ...
        success = supabase.update_memory(memory_data)
    else:
        # Create new memory
        memory_id = supabase.create_memory(memory_data)
...
def delete_memory(current_user, memory_id):
    """Delete a memory."""
    success = supabase.delete_memory(memory_id)
    return jsonify({'success': success})

# Theamal routes
@app.route('/api/theamal', methods=['GET'])
@token_required
def get_theamal_entries(current_user):
    """Get all theamal entries."""
    trait = request.args.get('trait')
    entries = supabase.get_theamal_entries(trait)
    return jsonify(entries)

@app.route('/api/theamal', methods=['POST'])
@token_required
def manage_theamal(current_user):
    """Create or update a theamal entry."""
    entry_data = request.json
    
    if 'id' in entry_data:
        # Update existing entry
        success = supabase.update_theamal_entry(entry_data)
    else:
        # Create new entry
        entry_id = supabase.create_theamal_entry(entry_data)
        success = entry_id is not None
    
    return jsonify({'success': success})

@app.route('/api/theamal/<int:entry_id>', methods=['DELETE'])
@token_required
def delete_theamal(current_user, entry_id):
    """Delete a theamal entry."""
    success = supabase.delete_theamal_entry(entry_id)
    return jsonify({'success': success})

@app.route('/api/theamal/<int:entry_id>/activate', methods=['POST'])
@token_required
def activate_theamal(current_user, entry_id):
    """Activate or deactivate a theamal entry."""
    data = request.json
    active = data.get('active', True)
    success = supabase.activate_theamal(entry_id, active)
    return jsonify({'success': success})
